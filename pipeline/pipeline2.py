    ...
    def _standardize_result(self, result, method_name, file_path):
        ...
        if isinstance(result, dict):          
            ...
            if 'is_stego' in result:
                ...
                standardized['classification'] = 'stego' if result['is_stego'] else 'clean'
                
                # LSB-specific fields
                if method_name == 'LSB':
                    standardized['entropy'] = result.get('entropy', {})
                    standardized['chi_square'] = result.get('chi_square', {})
                    standardized['z_score'] = result.get('z_score', 0)
            ...
            # DCT analyser format - Updated handling
                ...
                if isinstance(result_value, str):
                    is_stego = result_value.lower() in ['stego', 'steganography', 'suspicious']
                    classification = result_value.lower()
                elif isinstance(result_value, bool):
                    is_stego = result_value
                    classification = 'stego' if result_value else 'clean'
                else:
                    is_stego = bool(result_value)
                    classification = 'stego' if result_value else 'clean'
                ...
                if method_name == 'DCT':
                    ...
                    standardized['suspicious_blocks'] = result.get('suspicious_blocks', 0)
                    standardized['total_blocks'] = result.get('total_blocks', 0)
            ...
            # CNN analyser format - Updated handling
                ...
                if isinstance(prediction, str):
                    is_stego = prediction.lower() in ['stego', 'steganography', 'suspicious']
                    classification = prediction.lower()
                elif isinstance(prediction, bool):
                    is_stego = prediction
                    classification = 'stego' if prediction else 'clean'
                else:                    
                ...
                # CNN-specific fields
                if method_name == 'CNN':
                    standardized['features_analysis'] = result.get('features_analysis', {})
                    standardized['prediction_details'] = result.get('prediction_details', {})
            ...
            # Handle additional possible formats for DCT/CNN
                ...
                if isinstance(classification_field, str):
                    is_stego = classification_field.lower() in ['stego', 'steganography', 'suspicious']
                    classification = classification_field.lower()
                else:
                    is_stego = bool(classification_field)
                    classification = 'stego' if classification_field else 'clean'
                ...
                for field in ['classification', 'result', 'prediction', 'label', 'class', 'output']:
                    if field in result:
                        ...
                        if isinstance(value, str):
                            if value.lower() in ['stego', 'steganography', 'suspicious']:
                                classification = 'stego'
                                is_stego = True
                                break
                            elif value.lower() in ['clean', 'cover', 'normal']:
                                classification = 'clean'
                                is_stego = False
                                break
                        elif isinstance(value, bool):
                            is_stego = value
                            classification = 'stego' if value else 'clean'
                            break
